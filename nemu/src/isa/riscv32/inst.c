...
// 算术运算指令 - M 扩展
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, temp = (int32_t)src1*(int32_t)src2, R(rd) = temp >> 32);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, temp = (int32_t)src1*(uint32_t )src2, R(rd) = temp >> 32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, temp = (uint32_t )src1*(uint32_t )src2, R(rd) = temp >> 32);
